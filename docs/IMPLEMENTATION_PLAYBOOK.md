...
#### Phase 0 HARD GATE: Compute & Storage Pre-Flight
> **This gate is mandatory. No experiments may launch until it passes.**

1. Run `gen_runtime_estimate.py --project-dir .`
2. Review `docs/estimated_runtime.md`
3. Check these thresholds:

| Check | Threshold | If Exceeded |
|-------|-----------|-------------|
| Any single experiment >2 hours | WARN | Require subsampling plan or explicit approval in DECISION_LOG |
| Total sequential time >24 hours | BLOCK | Require parallelization plan or phased execution |
| Peak memory >70% available RAM | BLOCK | Require subsampling, sequential execution, or larger machine |
| SVM-RBF on >50K rows | BLOCK | Must subsample to ≤50K or use --force-full with justification |

4. Document estimates in PROJECT_BRIEF §2d
5. Record any subsampling decisions in ENVIRONMENT_CONTRACT §Compute Constraints

> **Gate pass criteria:** All BLOCKs resolved (subsampled, phased, or justified). WARNs acknowledged in DECISION_LOG.
